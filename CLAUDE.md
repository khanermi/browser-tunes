...
озвучки/плеера, с переносом фокуса на плеер. Псевдо-fullscreen и перехват PiP
...
- **Плеер — cross-origin iframe**, поэтому в `@match` кроме `old.yummyani.me`
  ...
  Внутри iframe скрипт только пробрасывает хоткей наверх через `postMessage`.
  ...
  чужих сайтах, ветка внутри iframe включается только если `document.referrer`
  — домен yummyani; иначе скрипт полностью инертен.
